...
AIハブは、ABCI上で大規模な汎用学習済みモデルの再利用を行うための、データセット・学習記録やAIモデル・実行環境イメージを共有できるプラットフォームです。AI資源リポジトリを中心に構成されており、いくつかのサービス群やツールセットを利用できます。AI資源リポジトリとは、膨大な学習データ、大規模汎用学習済モデル、学習処理系のコンテナイメージ、それらを活用するJupyter Notebookレシピ等のAI資源を一元的に管理する場所であり、Amazon S3互換オブジェクトストレージであるABCIクラウドストレージがその役割を果たします。
